...
export class GoodsReceiptDetailsComponent implements OnInit {
    ...
    printSelectedReports(value) {
        ...
        let grnos = "";
        let errMsg = "";
        for (let printIndex of this.exportArrayList) {
            grnos = grnos + printIndex.gr_num + ",";
        }
        grnos = grnos.substring(0, grnos.lastIndexOf(","));
        if (grnos.length == 0) {
            errMsg = "Please select atleast one GR No";
        }
        if (grnos.length != 0) {
            let url: any = this.environment.getRequiredApi("print_billing_list") + "?org_id=" + this.orgId + "&bp_id=" + this.bpId + "&im_v_vbeln=" + grnos + "&access_obj_id=" + this.accessObjectId + "&access_token=" + localStorage.getItem("token") + "&action=G";
            window.open(url, '_blank');
        } else {
            this.commonService.responseMessages("", errMsg, "warning");
        }
