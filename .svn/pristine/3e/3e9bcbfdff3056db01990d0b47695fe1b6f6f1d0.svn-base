...
export class RecurringOrderDetailsComponent implements OnInit {
    ...
    getAllMaterialList() {
        ...
        $("#black-overlay").show();
        let allmaterial = " ";
        let url: any = this.environment.getRequiredApi("find_matnr_num") + "?org_id=" + this.orgId + "&bp_id=" + this.bpId + "&material=" + allmaterial + "&";
        this.commonService.getData(url, "GET", '', this.accessObjectId)
            .subscribe((response) => {
                ...
                if (response.status == 1) {
                    ...
                    this.materialist = response["data"].ex_mat_list;
                }
                $('#loadingIcon').hide();
                $("#black-overlay").hide();
            });
            ...
            .subscribe((response) => {
                ...
                if (response.status == 1) {
                    ...
                    if (name != "" || name != "null" || name != undefined) {
                        let txt_nameid = "";
                        for (let types of this.recOrderList) {
                            if (types.value.trim() == name.trim()) {
                                txt_nameid = types.configValId;
                                this.recurringorderid = txt_nameid;
                                // $('#grp_orderlist').val(txt_nameid);
                            }
                        }
    ...
    changeOrderType() {
        ...
        this.commonService.getData(url, "GET", '', this.accessObjectId).subscribe((response) => {
            ...
            if (response.status == 1) {
                ...
                //                this.configvalarray.forEach(currentItem => {
                //                    this.materialist.forEach(matItem => {
                //                        if (currentItem.key == matItem.mat_num) {
                //                            this.configValSampleList[newcount].desc = matItem.mat_name;
                //                            this.configValSampleList[newcount].uom = matItem.uom;
                //
                //                        }
                //                    });
                //                    newcount++;
    ...
    checkboxsizecondition() {
        for (let index of this.configValSampleList) {
            if (index.mat_num != "" && index.qty != "") {
                ...
                this.materialist.forEach(matItem => {
                    if (index['mat_num'] == matItem.mat_num) {
                        index.desc = matItem.mat_name;
                        index.uom = matItem.uom;
