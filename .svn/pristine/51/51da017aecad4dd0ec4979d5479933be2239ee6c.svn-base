  ...
  ngOnInit() { 
    ...
    submituserDetails(){
     ...
     $("#black-overlay").show();
        ...
        let bpIdArray = this.payload.bpId.split("|");
        this.payload.bpId=bpIdArray[0];
        this.payload.bpType=bpIdArray[1];
        ...
        if(this.moduleName == "assigedUser"){
           ...
           this.exitedUSerDetails.mobile =  this.addUserForm.value.mobile;
             let bpIdArray = this.exitedUSerDetails.bpId.split("|");
            this.exitedUSerDetails.bpId=bpIdArray[0];
            this.exitedUSerDetails.bpType=bpIdArray[1];
    ...
    unlockUser(userId,action,firstName){
        let url = this.environment.getRequiredApi('unlock_user').replace("{action}", action) + "?user_id=" + userId + "&org_id=" + localStorage.getItem("orgId") + "&";
        this.reqmethod = 'POST';
        if(action == "LOCK"){
            

            swal({
                title: "Do you want to Lock this user '" + firstName + "' ?",
                //type: "warning",
                showCancelButton: true,
                confirmButtonClass: "btn-default btn-primary-custom login-button-export",
                cancelButtonClass: "btn-danger btn-danger-custom ",
            ...
            () => {
                this.commonService.getData(url, this.reqmethod, '', localStorage.getItem('Manage Users'))
                    .subscribe((response) => {
        ...
        /*if($(id+index).hasClass("fa fa-unlock-alt") && $("#title-change"+index).attr("title") =="Unlock"){
           $(id+index).removeClass("fa fa-unlock-alt");
           $(id+index).addClass("fa fa-lock");
           $("#title-change"+index).attr("title","lock")
               
       }else{
           $(id+index).removeClass("fa fa-lock");
           $(id+index).addClass("fa fa-unlock-alt");
           $("#title-change"+index).attr("title","Unlock")
       }*/
    }
}
