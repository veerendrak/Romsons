    ...
    constructor(private http: Http,
             ...
             this.commonService.getData(url, 'POST', this.addRoleManagementForm.value, this.accessObjId)
                         .subscribe((data) => {
                                 ...
                                 if(data.status==1){
                                     ...
                                     this.getRoles();
                                     this.commonService.responseMessages( '',this.messagesService.manageRoles.role_added, 'success' );
         ...
         if (this.updateRoleManagementForm.valid) {
             ...
             this.commonService.getData(url,"PUT",this.updateRoleManagementForm.value,this.accessObjId).subscribe((data) =>{
                 ...
                 if(data.status==1){
                     ...
                     this.getRoles();
                     this.commonService.responseMessages( '',this.messagesService.manageRoles.edit_role_name, 'success' );
                     this.addRoleManagementForm.reset();
                     }
             }); 
         }
         ...
         if (this.disableRoleManagementForm.valid) {
             ...
             this.commonService.getData(url,"DELETE","",this.accessObjId).subscribe((data) =>{
                 ...
                 if(data.status==1){
                     ...
                     this.getRoles();
                     this.commonService.responseMessages( '',this.messagesService.manageRoles.delete_role, 'success' );
                     this.addRoleManagementForm.reset();
                     }
             }); 
         }
     ...
     ngOnInit() {
         if (localStorage.getItem("token") != null) {
             console.log("----inside")
             /* this.requiredUrl = this.environment.getRequiredApi('om_get_org_roles')+"?action=DETAILS&role_type=ALL&org_id="+this.orgId+"&";
             this.commonService.getData(this.requiredUrl,"GET","",this.accessObjId).subscribe((response) =>{
                 if(response.status==0){
                     this.orgRoleslst= response["data"].roles; 
                 }
                                 ...
                                 if (this.roleList.roles != null) {
                                     this.roleList.roles.forEach(element => {
                                        ...
                                        this.rolesDeatils.accessObjs.forEach(currentItem=>{
                                            ...
                                            if (currentItem.hasOwnProperty("childObjs")){
 ...
 openSlideBlock(arrowId){
  ...
  updateRolePermission(roles,roleId,event,roleName){
      ...
      if(event.target.checked){
         ...
         docObject['childObjs']=[];
         if(roles.hasOwnProperty('childObjs')){
                         ...
                         .subscribe((data) => {
                             ...
                             if(data.status==1){
                               this.commonService.responseMessages( '',data.message, 'warning' );  
                                     ...
                                     this.getRoles();
                                     this.commonService.responseMessages( '','Role permissions are successfully updated', 'success' );
                                     }
                         });
  }
}
