...
export class ConfigValuesDetailsComponent implements OnInit {
    ...
    configValuesAppenedArray:any=[];
    disableNextButton:boolean;
    disablePrevButton:boolean;
    configValAddList: any;
    configValAddObj: any;
    totColspan:number;
    popupheaderList:any={};
    refreshflag:boolean;
    ...
    constructor(private http: Http, private formBuilder: FormBuilder,private activatedRoute:ActivatedRoute,
        ...
        this.resp_pg_num = 0;
        this.groupText="";
        this.configValuesAppenedArray=[];
        this.disableNextButton=true;
        this.disablePrevButton=true;
        this.configValAddList=[];
        this.configValAddObj={};
        this.totColspan=0;
        this.popupheaderList={};
    ...
    ngOnInit() {
        ...
        setTimeout(() => {
         ...
         for (let index = 0; index < 1; index++) {
            this.configValAddObj = {};
            this.configValAddObj["parentValId"] = "";
            this.configValAddObj["key"] = "";
            this.configValAddObj["value"] = "";
            this.configValAddObj["addField1"] = "";
            this.configValAddObj["addField2"] = "";
            this.configValAddObj["addField3"] = "";
            this.configValAddObj["addField4"] = "";
            this.configValAddObj["addField5"] = "";
            this.configValAddObj["addField6"] = "";
            this.configValAddObj["addField7"] = "";
            this.configValAddObj["addField8"] = "";
            this.configValAddObj["addField9"] = "";
            this.configValAddObj["addField10"] = "";
          
            this.configValAddObj["bpId"] = "";
            this.configValAddObj["configGroupId"] = "";
            this.configValAddObj["configValId"] = "";

            this.configValAddObj["displayRow"] = true;
            this.configValAddObj["mark_as_delete"] = false;
            

            this.configValAddList.push(this.configValAddObj);
        }
    ...
    getCongifGrpHeaders(groupText){
        ...
        this.configGrpList.forEach((items)=>{
            if(items.group_name == groupText){
                this.headerListOnConfigGrp =$.extend(true, {}, items);
                 this.headerListOnConfigGrp["colspan"]=0;
                ...
                for(let index of this.headerArr ){
                   ...
                   this.headerListOnConfigGrp["colspan"] = count++;    
                    }    
                }
                //addField1
       ...
       for(let index of this.headerArr ){
        ...
        this.totColspan = this.headerListOnConfigGrp.colspan+4;
        this.popupheaderList =this.headerListOnConfigGrp;
        console.log(this.popupheaderList);
        ...
        if (this.groupText != "" || this.groupText.length != 0) {
            ...
            this.commonService.getData(url, "GET", "", this.accessObjectId).subscribe((response) => {
                ...
                if (response.status == 0) {
                   ...
                   // }
                    this.configValuesAppenedArray = this.configValSampleList.concat(response.data.configValues);
            ...
            let urlGrp: any = this.environment.getRequiredApi("config_value_list") + "?group=" + this.groupText + "&";
            
            this.commonService.getData(url, "GET", "", this.accessObjectId).subscribe((response) => {
                                   
                if (response.status == 0) {
                  this.allCfgValsList=response.data.configValues;
                    console.log(this.allCfgValsList);
                } 
            });
    ...
    assignPopObject(value, index) {
       
        this.configValAddList[index]['parentValId'] = value;
    }

   AddMore(){
        let length: any = $("#pop-conf-grp-table").find('tbody').find('tr').length;
        if (length >= 5) {
    ...
    saveConfigValPop(action){
        
    console.log(this.configValAddList)
        let saveArray: any = [];
        let index = 0; let bflag: boolean = true;
        let confGrpParent = $( "#confg_grps_id option:selected" ).val().trim();
         let confGrpText = $( "#confg_grps_id option:selected" ).text().trim();
        let keyArray:any=[];
       
        for (let items of this.configValAddList) {
            let drpdwnText =  $("#pop-conf-grp-list-"+index+" option:selected" ).text().toLowerCase();
            let parentValId =  $("#pop-conf-grp-list-"+index+" option:selected" ).val();
             let count=0;
            if (items.displayRow ) {
                 index++
                if (items.hasOwnProperty("configGroupId")) {
                    items["parentValId"] = parentValId;
                     ...
                     items["configGroupId"] = confGrpText;
        ...
        if (bflag) {
           ...
           let dupFlag=false;
            for(var i=0;i<=keyArray.length;i++){
                for(var j=0;j<this.allCfgValsList.length;j++){
                    if(keyArray[i] == this.allCfgValsList[j].key){
                        dupFlag = true;
                        this.commonService.responseMessages("", "Key already exists", "warning");
                        break;
                    }  
                }
                if (dupFlag)
            ...
            if(dupFlag){return false;}
            // End Duplicate check
            
            this.configValAddList = saveArray;
        } else {
            return false;
        }
        ...
        this.configValAddList.forEach((items) => {
            items["displayRow"] = true;
        ...
        this.commonService.getData(url, 'POST', payload, '')
            .subscribe((response) => {
                ...
                if (response.status == 0) {
                    this.setParentList(confGrpParent,confGrpText);
                    if(action=="p"){
                    this.commonService.responseMessages("", response.message, "success");
                        this.closeModal('displayErrorsModal');
                    }
  ...
  onWindowScroll() {
    ...
    loadMoreRecords(){
        if ( this.configValuesAppenedArray.length <= this.ttl_cnt) {
                 this.scrollCount++;
