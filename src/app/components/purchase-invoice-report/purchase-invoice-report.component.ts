...
export class PurchaseInvoiceReportComponent implements OnInit {
                ...
                if (response.status == '1') {
                    ...
                    if (response.data.hasOwnProperty('ex_pi_report_list')) {
                        ...
                        for (let index of this.poreportdetailsList) {
                            ...
                            if (index.status == 'Generated Invoice') {
                            ...
                            if (index.status.includes('Cancellation of')) {
                                this.cngrossAmt = this.cngrossAmt + parseFloat(index.amount);
                                for (let item of index.pi_item_details) {
                                    if (item.hasOwnProperty('price')) {
                                        this.cnnetAmt = this.cnnetAmt + parseFloat(item.price);
                                    }
                                    if (item.hasOwnProperty('igst_value')) {
                                        this.cnigstAmt = this.cnigstAmt + parseFloat(item.igst_value);
                                    }
                                    if (item.hasOwnProperty('cgst_value')) {
                                        this.cncgstAmt = this.cncgstAmt + parseFloat(item.cgst_value);
                                    }
                                    if (item.hasOwnProperty('sgst_value')) {
                                        this.cnsgstAmt = this.cnsgstAmt + parseFloat(item.sgst_value);
                                    }
                                ...
                                if (element.vendor_name == index) {
                                    for (let item of element.pi_item_details) {
                                        if (element.status == 'Generated Invoice') {
                                            ...
                                            if (element.status.includes('Cancellation of')) {
    ...
    applyTable() {
        setTimeout(() => {
            $("#datatablediv-piSummaryItem").DataTable({
                ...
                { type: 'currency', "targets": [4,13,14,15,16,17] }]
            ...
            $("#datatablediv-inv_id").DataTable({
                ...
                { type: 'currency', "targets": [7,8,9,10,11] }]
    ...
    exportExcel() {
        ...
        if (this.poreportdetailsList.length != 0) {
            ...
            if (filterOption == 'piSummaryItem') {
                columns = ['inv_id', 'status', 'vendor_name', 'inv_date', 'amount', 'mat_id', 'mat_desc', 'qty', 'uom', 'batch', 'po_number', 'del_note', 'gi_no', 'price', 'cgst_value', 'cgst_value', 'igst_value', 'net_price'];
                columnHeaders = ['Invoice No', 'Status', 'Vendor Name', 'Invoice Date', 'Amount', 'Material', 'Description', 'Qty', 'UOM', 'Batch Number', 'PO No', 'Del No', 'GI No', 'Net Amount', 'S/UGST', 'CGST', 'IGST', 'Total Value'];
            ...
            if (filterOption == 'inv_id') {
                ...
                columnHeaders = ['Invoice No', 'Status', 'Vendor Name', 'Invoice Date', 'PO No', 'Del No', 'GI No', 'Net Amount', 'IGST', 'CGST', 'S/UGST', 'Total PI Amount'];
    ...
    applyDataTable(id, tableId) {
        setTimeout(() => {
            ...
            $("#black-overlay").hide();
            var targetArray=[];
            if(tableId=='piSummaryItem'){
               targetArray=[4,13,14,15,16,17]
            }
            if(tableId=='inv_id'){
               targetArray=[7,8,9,10,11]
            }
            if(tableId=='mat_id'){
               targetArray=[7,8,9,10,11]
            }
    ...
    displaypurchaseinvoicedetails(purId, delno, fiscal_year) {
        this.commonService.saleType = 'Edit';
        const path: any = "invoicedetails/editInvoice";
        this.router.navigate([path], { queryParams: { "po_inv_num": purId, "del_number": delno, "fiscal": fiscal_year, "action": "DIS" } });
    }
}
