...
@Component( {
    ...
    styles: ['.fa-cog { font-size: 22px !important;margin-top:-3px;margin-left:5px }'],
    providers: []
} )
export class MenuLeftComponent implements OnInit {
    ...
    ngOnInit() {
        let url = this.environment.getRequiredApi('get_left_side_menu')+"?org_id=" +this.orgId +"&bp_id="+this.bpId+"&";
        this.commonService.getData( url, 'GET', '', '')
            .subscribe(( response ) => {
                if(response.status!=1){
                this.leftMenu = response["data"].accessObjs;
                ...
                $( function() {
                    ...
                    $( '.cat__menu-left__action--menu-toggle' ).on( 'click', function() {
                        ...
                        if($('body').hasClass('cat__menu-left--visible')){
                            ...
                            var width=$("#mainContent").css("width");
                            $(".outbound-footer").css("width",width);
                            $(".table").attr('style','display:block;overflow-x:auto;overflow-y:hidden;');
                            $("#goods-receipt-order-table").attr('style','display:block !important;overflow-x:auto;overflow-y:hidden;');
                            $("#stock-balance-table").removeAttr('style');
                            ...
                            if(localStorage.getItem('roleName')=='KAD User'){
                                ...
                                $("#goods-receipt-order-table").removeAttr('style');
                                $("#invoice-order-table").removeAttr('style');
                            }
                        }
