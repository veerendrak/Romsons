...
export class StockBalanceComponent implements OnInit {
  ...
  constructor(private http: Http, private formBuilder: FormBuilder,
      ...
      this.accessObjectId="d480064c-1194-11e8-b642-0ed5f89f718b";
      this.pageNum=1;
      this.pageSize=10;
      this.sortBy=null;
          ...
          if ($("#stockFile").val().toLowerCase().indexOf(".xlsx") > 0 || $("#stockFile").val().toLowerCase().indexOf(".xls") > 0){
              ...
              if ( typeof ( FileReader ) != "undefined" ) {
                  ...
                  reader.onload = function( e: any ) {
                      ...
                      try {
                          if (xlsxflag) {  
                           workbook = XLSX.read(rowslist, { type: 'binary' });  
                      }  
                      else {  
                           workbook = XLS.read(rowslist, { type: 'binary' });  
                      } 

                      } catch (error) {
                          console.log("______**Error**______")
                          var message = error.message;
                          $.notify({
                              title: '<strong></strong>',
                              message: "Corrupted File.Please choose a valid file"
                          }, {
                                  type: 'warning'
                              });
                            $( "#loadingIcon" ).hide();
                          ...
                          setTimeout(function(){
                            ...
                            //render: $.fn.dataTable.render.ellipsis()
                                  render: function( data, type, row ) {
                                    ...
                                    return data.length > 13 ? data.substr(0, 15) + '...' : data;
                                 }
                              }],
  ...
  getStockBalanceDetails(){
      ...
      if(this.searchFilterByMaterialCd!=null && this.searchFilterByMaterialCd!=undefined){
          url=this.environment.getRequiredApi("get_stock_balance_details")+"?org_id="+this.orgId+"&pg_num="+this.pageNum+"&pg_size="+this.pageSize+"&sort_by="+this.sortBy+"&search_by="+this.searchFilterByMaterialCd.trim()+"&";
      }else{
...
lockStockBalanceConfirm(){
    if((this.stockBalanceStatus=="L" && this.stockBalanceListLength!=0) || this.stockBalanceStatus=="D"){
        
        return false;
    }
    swal( {
            title: "Do you want to lock this stock balance ?",
            //type: "warning",
...
lockStockBalance(){
    ...
    this.commonService.getData(url, "PUT", "", this.accessObjectId).subscribe(response=>{
        if(response.status==0){
            this.commonService.responseMessages("", response.message, "success");
            this.getStockBalanceDetails();
        }else{
