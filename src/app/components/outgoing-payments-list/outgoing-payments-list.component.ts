...
export class OutgoingPaymentsListComponent implements OnInit {
    ...
    getoutgoingpaymentlist() {
        ...
        this.url = this.environment.getRequiredApi('get_outgoing_payment_list') + "?bp_id=" + this.bpId + "&org_id=" + this.orgId + "&from_date=" + fromDate + "&to_date=" + toDate + "&";
    ...
    exportExcel() {
        ...
        if (this.exportArrayList.length != 0) {
            for (let index of this.exportArrayList) {
                ...
                if (!index.hasOwnProperty('vendor_account')) {
            ...
            let columns: any = ['payment_id', 'vendor_account', 'vendor_name', 'document_date', 'posting_date', 'amount']
            let columnHeaders: any = ['Payment ID', 'Vendor Account', 'Vendor Name', 'Document Date', 'Posting Date', 'Amount']
            let jsonData: any = JSON.stringify(jsonArrrayData);
            this.excelService.JSONToCSVConvertor(jsonArrrayData, "", "Outgoing Payment list", columns, columnHeaders, 'Outgoing Payment');
        } else {
    ...
    cancelPayment(tableId) {
        ...
        this.cancelPayload["object"] = "OUTGCP";
        ...
        swal({
            ...
            () => {
                if (this.checkboxLength == 0) {
                    ...
                    $('#loadingIcon').show();
                    $("#black-overlay").show();
                    let url: any = this.environment.getRequiredApi("outgoing_payment_cancel") + "?";
                    this.commonService.getData(url, "POST", this.cancelPayload, this.accessObjectId).subscribe(response => {
                        ...
                        if (response.status == 0) {
                            ...
                            if (this.errorLogs.length == 0) {
                                this.commonService.responseMessages("", "Payment is cancelled", "success");
                                this.indeterminate = false;
                                this.checked = false;
                                this.getoutgoingpaymentlist();
                            } else {
                                this.cancelPaymentFlag = true;
